...
class SubmitStreamView(APIView):
    ...
    def post(self, request):
        ...
        if is_submit and not request.user.is_authenticated:
            return Response({"error": "Authentication required for submission"}, status=status.HTTP_401_UNAUTHORIZED)
        ...
        if is_submit:
            ...
            testcases = Testcase.objects.filter(problem_id=problem_id, display_testcase=True).order_by('id')
        ...
        if codeblock:
            ...
            full_source_code = f"{codeblock.imports}\n\n{raw_source_code}\n\n{codeblock.runner_code}"
        else:
        ...
        def generator():
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = {executor.submit(run_testcase_internal, base_judge0_payload, tc): i for i, tc in enumerate(testcases)}
                ...
                if compilation_error:
                    ...
                    for r in results:
                        if not r.get("is_accepted"):
                            final_status = JUDGE0_STATUS_MAP.get(r.get("status", {}).get("id"), AnswerStatus.WRONG_ANSWER)
                            break
            ...
            yield json.dumps({
                "status": "complete", 
                "total_status": "Accepted" if total_passed and not compilation_error else "Error",
