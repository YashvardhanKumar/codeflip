      ...
      <DropdownMenuTrigger asChild>
        <div className="flex items-center gap-2 cursor-pointer group">
          <Avatar className="size-8 border border-slate-200 dark:border-surface-border transition-transform group-hover:scale-110">
            <AvatarImage src={user.profile_picture_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.username}`} />
      ...
      <DropdownMenuContent align="end" className="w-56 border-slate-200 dark:border-surface-border bg-white dark:bg-background-dark">
        <DropdownMenuLabel>
          <div className="flex flex-col">
            <span className="font-bold text-slate-900 dark:text-white">{user.name || user.username}</span>
        ...
        <DropdownMenuItem asChild className="cursor-pointer hover:bg-slate-100 dark:hover:bg-surface-border">
          <Link href="/profile">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <button className={cn(
              ...
              isProblem ? "text-gray-400 hover:text-white" : "text-slate-500 dark:text-text-secondary hover:text-slate-900 dark:hover:text-white"
            )}>
              <span className="material-symbols-outlined text-xl">notifications</span>
              {isProblem && <span className="absolute top-0 right-0 size-2 bg-red-500 rounded-full" />}
            </button>
