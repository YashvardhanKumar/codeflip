...
export default function SolutionsTab({ problem, onViewSolution }: Props) {
  ...
  const filteredSolutions = useMemo(() => {
    return solutions.filter(sol => {
      const matchesSearch = 
        ...
        sol.tags.some(t => t.tags.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  if (error) return <div className="py-10 text-red-500">Failed to load solutions.</div>;
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        >
          ...
          {selectedTags.length > 0 && (
            <Badge variant="secondary" className="ml-1 h-4 px-1 text-[10px] bg-primary text-white">
              {selectedTags.length}
            </Badge>
          )}
        </Button>
        <Link href={`/problems/${problem.id}/write-solution`}>
          <Button size="sm" className="h-9 bg-primary hover:bg-primary/90 text-xs gap-2 font-bold">
            <Plus size={14} />
            Write Solution
          </Button>
        </Link>
      ...
      {isFilterOpen && (
        <div className="p-4 bg-surface-dark rounded-xl border border-surface-border animate-in slide-in-from-top-2 duration-200">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-white uppercase tracking-wider">Filter by Tags</span>
            <Button variant="ghost" size="sm" className="h-6 text-[10px]" onClick={() => setSelectedTags([])}>Clear All</Button>
          </div>
          <div className="flex flex-wrap gap-2">
            {allTags.map(tag => (
              <Badge 
                key={tag.id}
                variant={selectedTags.includes(tag.tags) ? "default" : "secondary"}
      ...
      <div className="space-y-3">
          ...
          >
            <div className="flex gap-4">
              <Avatar className="size-10 rounded-lg border border-surface-border group-hover:border-primary/30 transition-colors">
                <AvatarImage src={sol.author.profile_picture_url ?? undefined} />
              ...
              <div className="flex-1 min-w-0 space-y-1">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-bold text-white truncate group-hover:text-primary transition-colors">{sol.title}</h4>
                  <span className="text-[10px] text-gray-600 shrink-0">{formatInUserTimezone(sol.created_at, 'MMM d, yyyy')}</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-500">
                  <span>{sol.author.username}</span>
                  <span className="size-1 rounded-full bg-gray-700" />
                  <div className="flex items-center gap-3">
                    <span className="flex items-center gap-1"><ThumbsUp size={12} /> {sol.upvote_count}</span>
                    <span className="flex items-center gap-1"><MessageSquare size={12} /> {sol.comment_count}</span>
                    <span className="flex items-center gap-1"><Eye size={12} /> {sol.views}</span>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {sol.tags.map(tag => (
                    <span key={tag.id} className="text-[10px] text-gray-500 bg-surface-border/50 px-1.5 py-0.5 rounded">#{tag.tags}</span>
                  ))}
                </div>
              </div>
              <ChevronRight size={18} className="text-gray-700 group-hover:text-primary transition-colors self-center shrink-0" />
            </div>
          </div>
        ))}
        ...
        {filteredSolutions.length === 0 && (
          <div className="py-20 text-center space-y-4 bg-surface-dark/30 rounded-2xl border border-dashed border-surface-border">
            ...
            <p className="text-gray-600 text-xs">Be the first to share your approach!</p>
          </div>
        )}
      </div>
    </div>
